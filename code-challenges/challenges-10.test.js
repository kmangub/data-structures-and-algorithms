...
const createServer = () => {
  ...
  const getBio = (request, response) => {
    response.send('My name is Karlo Mangubat and I love to play video games.');
  };
  const getFood = (request, response) => {
    let foods = ['ramen', 'tacos', 'beef brisket'];
    response.send(foods);
  };
...
const count = (target, input) => {
  ...
  input.map(function (val, idx) {
    // console.log(val);
    val.map(function (int, index) {
      // console.log(int);
      if (int === target) {
        newCount++;
      }
