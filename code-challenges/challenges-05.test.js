...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(value => {
    ...
    let $cloned = $(`<section>${html}</section>`);
    $cloned.find('h2').text(value.name);
    $cloned.find('h3').text(value.height);
    $cloned.find('p').text(value.eye_color);
    $('main').append($cloned);
  });
};

// collaborated with Grace Choi
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    let pencilLength = str.slice(i, str.length);
    result.push(pencilLength);
  }
  result.push('');
  return result;
};
...
// includes, indexOf must be pounds, cups, gallons
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  recipe.ingredients.forEach((food) => {
    if (food.includes('sized')) {
      let sized = food.indexOf('sized');
      result.push(food.slice(sized + 6, food.length));
      ...
      let gallons = food.indexOf('gallons');
      result.push(food.slice(gallons + 8, food.length));
    } else if (food.includes('cups')) {
      let cups = food.indexOf('cups');
      result.push(food.slice(cups + 5, food.length));
    } else if (food.includes('pound')) {
      let pound = food.indexOf('pound');
      result.push(food.slice(pound + 6, food.length));
    }
  });
  // console.log(result);
  return result;
