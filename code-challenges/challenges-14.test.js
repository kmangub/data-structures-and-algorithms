...
const toTitleCase = (arr) => {
  // Solution code here...
  let upperCaseArr = arr.map(word => word.charAt(0).toUpperCase() + word.slice(1));
  return upperCaseArr;
};

// I got the answer got from https://stackoverflow.com/questions/1026069/how-do-i-make-the-first-letter-of-a-string-uppercase-in-javascript
...
let biggerThanLuke = (arr) => {
  ...
  let newArr = [];
  arr.map(character => {
    if (character.mass > 78) {
      newArr.push(character.name);
    }
  });
  let combineArr = ''.concat(newArr);
  let finalStr = combineArr.replace(/,/, ' - ');
  return finalStr;
};
